...
_nextTarget = nextTarget4;
_score = 0;
_targets = [pt4, pt4_1, pt4_2];
_loop = 1;
_loop2 = 1;
_shooter = nearestObject [range4, "player"];

{_x  animate["terc",1]} forEach _targets; //puts the targets down before the start

nopop=true;

"Setting up Range 4" remoteExec ["hint", _shooter]; // Range setup hints to player who called the action.
sleep 2;
"Get Ready Range 4..." remoteExec ["hint", _shooter];
sleep 2;
...
execVM "scripts\timer4.sqf"; //extra script to set time for second target group
...
"Range 4 Complete" remoteExec ["hint", _shooter];
sleep 2;
