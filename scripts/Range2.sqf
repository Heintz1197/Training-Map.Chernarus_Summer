...
_nextTarget = nextTarget2;
_score = 0;
_targets = [pt2, pt2_1, pt2_2];
_loop = 1;
_loop2 = 1;
_shooter = nearestObject [range2, "player"];

{_x  animate["terc",1]} forEach _targets; //puts the targets down before the start

nopop=true;

"Setting up Range 2" remoteExec ["hint", _shooter]; // Range setup hints to player who called the action.
sleep 2;
"Get Ready Range 2..." remoteExec ["hint", _shooter];
sleep 2;
...
execVM "scripts\timer2.sqf"; //extra script to set time for second target group
...
"Range 2 Complete" remoteExec ["hint", _shooter];
sleep 2;
